...
class AbonnementController extends Controller
{
    public function index(Request $request)
    {
        ...
        $abonnements = Abonnement::where('adherent_id', $user->id)
    ...
    public function souscrire(Request $request)
    {
        ...
        Abonnement::where('adherent_id', $user->id)
            ->where('statut', 'actif')
    ...
    public function check(Request $request)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['hasAbonnement' => false], 200);
        }
    ...
    public function suspendre($id)
    {
        ...
        return response()->json(['message' => 'Abonnement suspendu', 'abonnement' => $abonnement]);
    }
    ...
    public function renouveler(Request $request, $id)
    {
        ...
        $request->validate(['duree_mois' => 'required|integer|min:1']);

        $abonnement->update([
            'date_fin' => \Carbon\Carbon::parse($abonnement->date_fin)->addMonths($request->duree_mois),
            'statut'   => 'actif',
        ]);

        return response()->json(['message' => 'Abonnement renouvelé', 'abonnement' => $abonnement]);
    }
    ...
    public function expireAutomatique()
    {
        ...
        return response()->json(['message' => "$count abonnements expirés automatiquement"]);
    }
